...
data <- read.csv("ukindex2.csv", stringsAsFactors = FALSE)
# data1 <- select(data, company_name, website, company_number, status, address, parent_company, parent_company_number)
...
server <- function(input, output) {
  ...
  output$mymap <- renderLeaflet({
    ...
    #%>% ADD BOUNDS FOR UK HERE
      # addCircleMarkers(lng = longditude, lat = latitude, popup = data$name, radius = 1, weight = 2, opacity = 0.5, fill= TRUE, fillOpacity = 0.2)
  })
